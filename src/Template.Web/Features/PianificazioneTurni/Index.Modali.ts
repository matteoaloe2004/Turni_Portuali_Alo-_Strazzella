...
namespace PianificazioneTurni {
    ...
    export interface IndexVueModel {
        ...
        sincronizzaPannelloDSS(deveEsserAperto: boolean): void;
    ...
    function agganciaChiusuraPannello(el: HTMLElement, self: any): void {
        if (chiusuraPannelloAgganciata) return;
    ...
    IndexVueModel.prototype.sincronizzaPannelloDSS = function (this: IndexVueModel, deveEsserAperto: boolean): void {
        const el = document.getElementById(ID_PANNELLO_DSS);
        if (!el || typeof bootstrap === 'undefined' || !bootstrap.Offcanvas) return;

        agganciaChiusuraPannello(el, this as any);
